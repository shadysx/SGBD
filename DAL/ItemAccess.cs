...
namespace DAL
{
    public class ItemAccess
    {
        public static List<Item> SelectAllItem(int idOrdered)
        {
            ...
            string query = $"select distinct o.ID_ORDERED, o.ID_PRODUCT, o.ID_ORDER_LINE, o.ORDER_LINE_QUANTITY, o.ORDER_LINE_BUYING_PRICE, s.ID_SHOP, s.SHOP_NAME, s.SHOP_ADDRESS, s.SHOP_CITY, s.SHOP_COUNTRY, p.PRODUCT_NAME, p.PRODUCT_TYPE, p.PRODUCT_DESCRIPTION, pic.PICTURE_URL from ORDER_LINE o inner join product p on p.id_product = o.id_product inner join picture pic on pic.id_product = p.id_product inner join shop s on s.ID_SHOP = o.ID_SHOP where o.ID_ORDERED = {idOrdered}";
